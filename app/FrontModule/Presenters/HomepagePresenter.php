...
class HomepagePresenter extends BasePresenter
{
	...
	protected function createComponentContactForm(): Form
	{
        ...
        $form->addText('hp', '')
            ...
            ->setHtmlAttribute('style', 'position:absolute;left:-9999px;top:-9999px;')
            ->setRequired(false);

        $form->addHidden('form_time', (string) time());
        ...
        $form->onValidate[] = function (Form $form): void {
            ...
            if (!empty($values->hp)) {
                $form->addError('Formulář se nepodařilo odeslat. Zkuste to prosím znovu.');
            }

            $createdAt = (int) $values->form_time;
            $now = time();
            $minSeconds = 3;
            $maxSeconds = 3600;

            if ($createdAt === 0 || ($now - $createdAt) < $minSeconds || ($now - $createdAt) > $maxSeconds) {
                $form->addError('Formulář byl odeslán příliš rychle nebo pozdě. Zkuste to prosím znovu.');
            }
        };
		...
		$form->onSuccess[] = function (Form $form, ArrayHash $values) {
            ...
            try {
                if (!empty($values)) {
                    ...
                    $mail->setFrom('noreply@kalcuc.cz', 'Kalcuc');
                    $mail->addTo('kalcuc@seznam.cz');
                    $mail->addReplyTo($values->email);
                    $mail->setSubject('Kalcuc.cz - Zpráva z kontaktního formuláře');
                    $mail->setHtmlBody(
                        "<b>Jméno a příjmení:</b> {$values->name}<br>" .
                        "<b>Telefonní číslo:</b> {$values->phone}<br>" .
                        "<b>Email:</b> {$values->email}<br><br>" .
                        "<b>Zpráva:</b><br>" . nl2br(htmlspecialchars($values->message))
                    );
    ...
    protected function createComponentCalculator(): Form {
        ...
        $form->addText('starting_position', 'Startovní pozice')
            ->setDefaultValue('Dukelská 1271, 438 01')
        ...
        $form->addInteger('waste_amount', 'Počet kubických metrů odpadních vod')
            ...
            ->setRequired();
        $form->addInteger('amount_price', 'Cena za množství')
            ->setHtmlAttribute('readonly');
